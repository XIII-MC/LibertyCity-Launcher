            ...
            for (File file : Objects.requireNonNull(modFolder.listFiles())) {
                if (!mods.contains(file.getName())) {
                    if(!whiteListedMods.contains(file.getName())) {
                        System.out.println("Does not exist on Server: " + file.getName());
                        file.delete();
                    } else {
                        System.out.println("Does not exist on Server, but is Whitelisted: " + file.getName());
                        try {
                            ...
                            if (!Arrays.equals(sha1, getSha1FromURL(fileURL))) {
                                System.out.println("File did not match: " + file.getName());
                                file.delete();
